...
2026-09-04, 1-1 / B-1).  With --max-offset N the window is fixed to
REF_1..REF_N regardless of the data, so datasets of one designed
experiment share a model shape (docs/design/mixture_doe.md section 2).
"""
...
def prepare(dataset_dir: str | Path, out_dir: str | Path,
    ...
    if max_offset is not None:
        ...
        if max_offset < 1:
            raise ValueError(f"--max-offset must be >= 1, got {max_offset}")
    fixed = max_offset is not None
    if test_dataset is not None:
        if window_from != "train" and not fixed:
            raise ValueError(
    ...
    # or only window_from).  A fixed --max-offset overrides the search.
    loaded: dict[str, list[tuple[dict, MetaGraph, int]]] = {}
    derived_max_offset = 1
    for split_name, info in manifest["splits"].items():
        ...
        for entry in info["samples"]:
            ...
            if not fixed and (window_from is None or split_name == window_from):
                derived_max_offset = max(derived_max_offset, needed)
            rows.append((entry, mg, needed))
        loaded[split_name] = rows
    if not fixed:
        max_offset = derived_max_offset
    ...
    if window_from is not None or fixed:
...
def _prepare_cross_dataset(source: Path, target: Path, out: Path,
    ...
    meta = {"max_offset": max_offset, "max_len": max_len, "max_len_source": "train",
            ...
            "window_from": "fixed" if fixed else "train", "sources": sources,
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument(
        "--max-offset", type=int, default=None, metavar="N",
        help="fix the REF window to REF_1..REF_N instead of deriving it "
