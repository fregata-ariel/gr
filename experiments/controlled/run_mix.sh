...
declare -A BUNDLE=( [layered]=tok_c_layered [structured]=tok_c_structured [depth1]=tok_c_depth_id [merge2]=tok_c_merge_id [balanced]=tok_c_balanced [mixed]=tok_c_mixed )
declare -A CFGS=( [layered]="base mask ptr" [structured]="base mask" [depth1]="base mask" [merge2]="base mask" [balanced]="base mask" [mixed]="base mask" )
declare -A TGTS=( [layered]="tok_c_lay2depth tok_c_lay2merge" [structured]="tok_c_str2depth tok_c_str2merge" [depth1]="tok_c_depth" [merge2]="tok_c_merge" [balanced]="" [mixed]="tok_c_mix2lay tok_c_mix2str tok_c_mix2spa tok_c_mix2depth tok_c_mix2merge" )
for src in mixed layered structured; do
  B=${BUNDLE[$src]}
  timeout 300 colab upload -s $SES training/train_ar.py /content/train_ar.py 2>&1 | tail -1
  timeout 300 colab upload -s $SES training/grammar_mask.py /content/grammar_mask.py 2>&1 | tail -1
