...
for i in $(seq 1 60); do
  out=$(timeout 120 colab new --gpu T4 -s cm 2>&1 | tail -1)
  echo "attempt $i $(date +%T): $out"
  if colab sessions 2>&1 | grep -q "^\[cm\]"; then echo "SESSION-OK $(date +%T)"; break; fi
  sleep 120
done
if colab sessions 2>&1 | grep -q "^\[cm\]"; then
  bash $S/c/run_mix.sh > $S/c/run_mix.log 2>&1
else
  echo "GIVE-UP $(date +%T)"
fi
