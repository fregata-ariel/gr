...
if [ -e data/tok_d24_p103_lay ]; then printf '%s\n' 'existing dataset/bundle requires verification: data/tok_d24_p103_lay' >&2; exit 2; fi
uv run python -m training.prepare_tokens --dataset data/d24_p103 --test-dataset data/s24_layered --max-offset 20 --out data/tok_d24_p103_lay
if [ -e data/tok_d24_p103_str ]; then printf '%s\n' 'existing dataset/bundle requires verification: data/tok_d24_p103_str' >&2; exit 2; fi
uv run python -m training.prepare_tokens --dataset data/d24_p103 --test-dataset data/s24_structured --max-offset 20 --out data/tok_d24_p103_str
if [ -e data/tok_d24_p103_spa ]; then printf '%s\n' 'existing dataset/bundle requires verification: data/tok_d24_p103_spa' >&2; exit 2; fi
uv run python -m training.prepare_tokens --dataset data/d24_p103 --test-dataset data/s24_spaghetti --max-offset 20 --out data/tok_d24_p103_spa
