...
colab upload -s ptr2 training/train_ar.py /content/train_ar.py
colab upload -s ptr2 training/grammar_mask.py /content/grammar_mask.py
for n in 24 32; do
  NAME=ptr2_n$n; DATA=sweep_n$n
  echo "=== $NAME start ==="
  for f in train.jsonl val.jsonl vocab.json meta.json; do
    colab upload -s ptr2 "data/tokens_$DATA/$f" "/content/$f"
  done
  colab exec -s ptr2 -f "$S/w_$NAME.py" --timeout 5400 2>&1 | grep -E "epoch   1 |early stop|wrote|Error|error|Traceback" | tail -6
  mkdir -p "runs/$NAME"
  for f in samples.json history.json model.pt val_scores.jsonl; do
    colab download -s ptr2 "/content/$NAME/$f" "runs/$NAME/$f"
  done
