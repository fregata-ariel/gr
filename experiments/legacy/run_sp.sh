#!/bin/bash
# 案 1 (--struct-pos) on the same data as sweep_n24 / sweep_n32
set -e
...
colab upload -s sp training/train_ar.py /content/train_ar.py
colab upload -s sp training/grammar_mask.py /content/grammar_mask.py
for n in 24 32; do
  NAME=sp_n$n; DATA=sweep_n$n
  echo "=== $NAME start ==="
  for f in train.jsonl val.jsonl vocab.json meta.json; do
    colab upload -s sp "data/tokens_$DATA/$f" "/content/$f"
  done
  colab exec -s sp -f "$S/w_$NAME.py" --timeout 5400 2>&1 | grep -E "early stop|wrote|Error|error" | tail -4
  mkdir -p "runs/$NAME"
  for f in samples.json history.json model.pt val_scores.jsonl; do
    colab download -s sp "/content/$NAME/$f" "runs/$NAME/$f"
  done
