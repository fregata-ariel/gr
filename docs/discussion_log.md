...
## セッション7: PoC-0.5 — 文法制約サンプリングとコーパス拡大 (2026-07-13)

### 方針決定

セッション6の診断（decode_error の約85%が ptr 制約違反、妥当率ギャップは過学習でなく
汎化課題）を受け、ユーザー決定: **(1) decode 整合の文法制約サンプリング + (2) コーパス拡大**
の両輪。制約は opt-in（`--constrained`）とし、非制約妥当率を「文法学習の正直な指標」として
測り続ける。

モデル委譲: 計画 = GPT-5.5 xhigh、実装 = GPT-5.4 high の2ジョブ分割。
（ユーザー指定の GPT-5.6 Sol/Terra/Luna は本アカウント未展開のため最近縁で代替。
5.6 プロンプトガイド要約は docs/gpt56_prompting_notes.md に取得済み — 移行時は
Sol=計画/Terra=実装、既存XMLプロンプトのまま effort を現行値で評価から）

### 実装（poc0/grammar.py + サンプラ/eval 配線）

- `GrammarTracker`: `_Parser` の受理規則を写像した不変状態機械（numpy のみ）。
  状態 = ITEM_TOP/ITEM_CHILD/PARENTS_NONLOOP/PARENTS_LOOP/EXPECT_CLOSE/DONE。
  非ループ親リストの「区切りトークン非消費」、OPEN 時点での親カウント増加、
  レベル毎 ptr 上限（昇順・重複禁止・完了アイテム数未満・語彙域 ≤31）まで再現。
  kind の引数個数は decode が検査しないため課さない（decode 整合原則）。
- **プロパティテスト両方向**: A1 = 敵対的 logits でのマスク下ロールアウトが全て decode
  通過（442状態）; A2 = prefix+候補の decode オラクル（エラー位置が候補より後 = 消費 = 合法）
  との合法集合一致（749状態）。オラクル不一致ゼロ。
- 非制約パスはスナップショットテストでバイト同一を保証。
- コーパス: seeds 5000/config で **95,988 ユニーク**（4.6倍、生成98秒、max ptr 17 /
  max 系列長 68 で語彙・系列長は変更不要と再確認）。

### 本走結果（T4、3000 steps ≈ 8エポック、4評価 各N=1000）
...
- **受入基準（制約時 ≥99.9%）達成**。empty_stream 0、length_cap 0。
- best val 0.516（PoC-0: 0.549）、final == best — 8エポックでは過学習が消滅し
  val loss は最後まで下降。
- データ4.6倍で非制約妥当率 84.5%→92.5%。残余75件は依然 ptr 系
  （前方/自己参照54、非昇順12、重複3）— ptr 計数の汎化はデータで大幅改善するが
  完全には到達しない。
- **文法マスクは分布を歪めない**: 制約付きの新規率・TVD が非制約と同水準
  （不正確率の質量が合法トークンへ再配分されるだけ）。

### PoC-0 総括

コア仮説「正準直列化されたメタグラフ DAG の分布は小型 decoder-only Transformer で
学習可能」は**支持**。3基準（妥当率・新規性・分布一致）は制約付きサンプリングで全達成。
成果物: runs/poc0_t4/, runs/poc0_t4_rerun/, runs/poc05_t4/（重みはローカルのみ）。

### 未決事項（更新）

- [x] PoC-0.5: 制約サンプリング + コーパス拡大 → 完了、全基準達成
- [ ] PoC-1 設計: グラフ条件付きバックボーン、エンコーダ位置符号化、
      ptr と位置埋め込みの結びつけ（pointer-network 風、ptr 汎化の構造的解決）
- [ ] 層内タイブレークの WL 精緻化（設計済み、据え置き）
- [ ] E. 重み共有戦略 / F. 分岐履歴 / Motif語彙粒度 / header正準化（不可約、据え置き）
