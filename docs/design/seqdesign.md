...
### 5.3 実機 campaign(n24、`experiments/mixture_doe/adaptive/`、2026-09-26)

- 設定: 候補 235(0.05 格子 + p7〜p10)、初期履歴 = DoE 12 点 × 3 seed(超過 NLL、target 固有基準)、目的 bal、TS k=3、
  seed 2。実体化は `materialize.py`、実行は runner(Colab L4、点ごとに別セッションで並列)。
- **ラウンド 1**(提案 p100 (0.35, 0.15, 0.50)、p101 (0, 0.30, 0.70)、p102 (0.60, 0.05, 0.35)、各 2 seed、6 run + 再スコア 12、
  3 セッション並列で 20 分、< 1 CU):

  | 点 | 実現 (lay, str, spa) | lay / str / spa | bal | max |
  |---|---|---|---|---|
  | p100 | (0.35, 0.14, 0.51) | −0.432 / −0.359 / −0.375 | −0.389 | −0.359 |
  | p101 | (0.00, 0.29, 0.71) | +0.177 / −0.393 / −0.388 | −0.201 | +0.177 |
  | p102 | (0.60, 0.06, 0.34) | −0.474 / −0.325 / −0.356 | −0.385 | −0.325 |

  内部 2 点の bal は DoE の重心(−0.386)や 2:1:1(−0.388)と同じ水準で「bal は内部で平坦」を再現。layered を含まない
  辺上の p101 は layered で +0.18 と悪く、辺・頂点の特異性も再現した。
- **ラウンド 2**: 42 観測から TS が提案したのは 3 回とも同じ p103 (0.25, 0.55, 0.20)(= 6 seed の反復)。structured 寄りの
  内部点で、事後の不確実性が残る領域を反復で確かめる挙動。実行中(§5.4 に追記)。
- 運用上の注意: `materialize.py` の generate.sh は context.dataset_version(campaign 作成時の HEAD)と HEAD の一致を要求するため、
  docs のコミットで HEAD が進むと失敗する。生成器のコードは不変なので guard を外して実行した。要改修: 比較対象を
  `git rev-parse HEAD:cfg_reducer`(生成器ツリーのハッシュ)にする。

### 5.4 実機 campaign ラウンド 2 と判定(2026-09-26)

- **ラウンド 2**: p103 (0.25, 0.56, 0.19) を 6 seed(3 セッション並列、10 分): lay −0.397 / str −0.415 / spa −0.310、
  bal **−0.374 ± 0.003**(seed SD)、max −0.310。structured 寄りに振ると spaghetti が最悪ケースになり、bal も内部の
  中央付近(−0.386〜−0.389)より 0.012 悪い。DoE の「spaghetti を減らさない」「bal は内部で平坦、端に寄ると
  悪化」を反復付きで再確認した。
- 48 観測後の GP 推奨(bal): 候補 115 (0.30, 0.20, 0.50)、潜在 95% 区間 -0.425568, -0.384507。次ラウンドを回すとすれば TS は
  (0.55, 0.15, 0.30)、(0.35, 0.65, 0)、(0.65, 0.30, 0.05) を提案する(layered 寄り・structured 寄りの未探索域)。
- **判定**: 逐次設計の仕組み(campaign → 提案 → 実体化 → runner → collect → observe → 報告)は実機で一巡し、
  既存 DoE の結論と矛盾しない提案・推奨を返した。A9-4 の「全パターンから乱択するベイズ的な方法」の初版として
  採用する。転用時の注意: (1) 等分散ノイズ GP は頂点の大きな seed 分散を内部に持ち込む(潜在区間は過信気味、
  §5.2)、(2) 同じ点の反復提案が出ることがある(設計どおり。seed 数で不確実性を詰める)、(3) 実体化の guard は
  生成器コードのハッシュ比較に修正済み(b594d56)。
- 費用: 2 ラウンド 12 run + 再スコア 16 件で 1 CU 未満、壁時計 30 分(3 セッション並列)。
