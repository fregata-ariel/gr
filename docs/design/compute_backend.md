...
- CU の照会手順: `HOME=<token のコピーを置いた隔離 HOME> uvx --from google-colab-cli==0.7.2 colab usage`
  ...
  配下でバックアップ対象外)。
- 追記(2026-09-26 18:09): L4 を 2 本稼働中に 3 本目の L4 `colab new` が 412 で 2 回続けて拒否され、20 分後に成功した。
  同時 3 本は「1 L4 + 2 T4」では即時成功したので、L4 は同時 2 本までか、直前に停止したセッションが一定時間
  カウントされる可能性がある(要検証)。3 本並列にするときは 3 本目を T4 にするか、ランナーの再試行(10 分間隔)に任せる。
