...
## A6. 外部レビュー対応(2026-09-04 回答)

1. main はこちらの変更を優先して merge する。
2. スコープは分割する(A: 本 PR / B: 対照実験 / C: 生成器 v2 + OOD)。
3. 案 2″ は暫定第一候補に改める。成績が良いことは無視できず、工夫も評価する。
4. GitHub Actions は延期する。
5. B は 18 run(3 構成 × 3 seed × 2 サイズ)で進める。
6. 窓なし pointer は C に回す。
7. Session 3 の GT 設計との接点は Issue に上げ、後から参照できるメモにする。

詳細と反映先: `docs/design/external_review_2026-09-04.md` §5。

---

## Q7. C(生成器 v2 / 構造 OOD)の設計判断(優先度: 高)

B の判定(案 2″ 不採用、既定 = 参照合法マスク)を受けた C の設計見直しは
`docs/design/generator_v2.md` にまとめた。判断事項は同 §7(Q7-1〜Q7-7)。

## A7. C の設計判断(2026-09-09 回答)

1. 3 族(layered / structured / spaghetti)で開始する。族は後で複数回差し替え・追加する
   ので、抽象化に注意すること。
2〜4, 6, 7. 承認。
5. n24 主体でよいが、節目では前回(構造スイープ)と同様に多くのノード数で実証する。
...
## A8. 計算バックエンドのルーター化(2026-09-16 回答)

3. 既定の振り分けは **Colab**。優先順は設定値であり、設計で固定するのは振り分けの
   粒度(Plan 単位で同一実行先に揃える)とフォールバックの有無。
...
## A9. 混合重みの実験計画(2026-09-16 回答)

1. 2:1:1 は設計点に含めず確認 run で検証する。
2. 合成応答に最良ケース y_min = min(y_lay, y_str, y_spa) を追加(y_bal / y_max / y_min の 3 本)。
3. 窓固定(`--max-offset`)は承認。終了後に n48 で同傾向かを調べる。
4. n48 の 5 点確認は承認。補足: 今後この実験系を転用する際は、原則として全パターンから乱択する
   ベイズ的な方法(適応的な設計点の選択)を入れる方針。今回の検証はモデルの特徴そのものの探査で
   あり、同時に実験の仕組みの validation を兼ねるので、まずは本計画で進める。
5. numpy と matplotlib を使う。
6. 混合族 spec(頂点は純族 spec)で生成、pool 合成案は不採用。

## Q10. 混合重みの既定値(優先度: 中)

n24 の混合物計画と確認 run(`mixture_doe.md` §8.1〜8.2)の結論: bal(3 target 平均)は重心〜(⅔,⅙,⅙)の
間で平坦、最悪ケース(layered test)は x_lay に線形に改善、代償は structured に線形。既定の重みを
どれにするか。
1. 重心 (⅓,⅓,⅓) のまま(均等重視、現状維持)。
2. (⅔,⅙,⅙)(最悪ケース最良: layered +0.044、structured +0.047、spaghetti +0.020 対 重心)。
3. 2:1:1 または 5:2:3(折衷: layered −0.03、structured +0.02〜0.03)。
推奨: n48 で同傾向なら 2(最悪ケースを抑える)。ただし用途で重みが決まる問題なので指定があれば従う。

補足(2026-09-17、正規化後、`mixture_doe.md` §9.1): 族固有の基準符号長を引くと、混合モデルの利得は
layered で最大・spaghetti で最小。bal は平坦のまま(重心 / 2:1:1 / 5:2:3 / 点 8 の差 ≤ 0.007)。正規化した
最悪ケースは spaghetti で決まり、layered を増やす点 8 は不利。推奨を改める: 既定は**重心のまま**
(案 1)、運用上 layered の生 NLL を抑えたいなら 2:1:1(案 3)。案 2 は推奨しない。

## Q11. Coder Web IDE への移行(優先度: 高)

背景: `docs/devenv.md`(範囲確定・バックアップ導入済み・restore.sh dry-run 済み)。Colab アダプタの実機試験は 2026-09-26 に通った
(`accept_colab_n12`、T4、セッション確保から RUN-DONE まで 1.5 分、セッションは自動解放)。以下は移行先の前提で決まる点です。

1. **Coder ワークスペースの計算資源**: GPU と Docker は使えますか? 使えなければローカルバックエンドは落とし、`colab` 既定のみで運用します
   (推奨: Colab Pro が使える間は colab のみ。Docker イメージ 7.4 GB を持ち込む必要がなくなります)。
2. **`data/`(2.4 GB)と `runs/`(0.8 GB)**: 移行先に持ち込むか、`data/` は spec から再生成するか。
   推奨: 両方持ち込む(restore.sh は既定で含める)。再生成は古いデータで当時のコミットが要るため、手間のわりに節約が小さい。
3. **scratchpad の実験メタデータを git 管理へ**: `sw/spec_*.json`、`doe/{specs,plan_*.json,obs*.json}`、`rt/plan_*.json`、集計スクリプト
   (合計約 2.3 MB)をリポジトリ内 `experiments/` に移し、コミットする案です。`/tmp` 依存が消え、移行と再現が git だけで済みます。
   基準符号長(13 MB、再計算可)と実行ログは gitignore のまま同じ場所に置きます。推奨: 採用。
